  ...
  update(audioState, nodalPoints) {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      if (beat > 0.7) {
        ...
        this.velocities[i * 2]     += (px / mag) * beat * 0.01;
        this.velocities[i * 2 + 1] += (py / mag) * beat * 0.01;
      }
      ...
      if (nodalPoints && nodalPoints.length > 0) {
        ...
        const step = Math.max(1, Math.floor(nodalPoints.length / 20));
